-- Reescritura de SOLO EL CUERPO de create_bulk_reservation (misma firma,
-- sin DROP): los mensajes de error por habitación, dentro del loop,
-- identificaban la habitación por su UUID (o no la identificaban en
-- absoluto) -- inútil para una recepcionista, y catastrófico para
-- payer_mode='client', donde el `raise;` deliberado (feat/booking-12,
-- atomicidad todo-o-nada) hace que ese mensaje crudo sea EXACTAMENTE lo
-- que ve la pantalla, sin ningún room_id de fallback (ver
-- BulkReservation.tsx, que sólo tiene `roomNumberOf` para las fallidas de
-- payer_mode='each_stay', nunca para la excepción re-lanzada). Fix:
-- resolver rooms.room_number UNA vez por vuelta del loop (v_room_number),
-- apenas se conoce v_room_id, y usarlo en TODOS los raise exception de la
-- vuelta que antes usaban v_room_id o ninguna referencia a la habitación.
